...
public class CityRepository {
    ...
    public CityRepository(List<String> cities, Random random) {
        ...
        for (String raw : cities) {
            ...
            if (!Character.isLetter(n.charAt(0))) continue;
        ...
        try (var in = CityRepository.class.getClassLoader().getResourceAsStream(resourcePath)) {
            ...
            try (BufferedReader br = new BufferedReader(new InputStreamReader(in, StandardCharsets.UTF_8))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (!trimmed.isEmpty()) cities.add(trimmed);
                }
            }
    ...
    public boolean contains(String normalizedCity) {
        if (normalizedCity == null || normalizedCity.isBlank()) return false;
    ...
    public Optional<String> pickAnswer(char requiredFirstLetter, Set<String> usedNormalized) {
        ...
        for (String c : candidates) {
            if (usedNormalized.contains(c)) continue;

            seen++;
            if (random.nextInt(seen) == 0) chosen = c;
        }
